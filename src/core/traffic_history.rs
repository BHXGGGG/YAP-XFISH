...
/// 把当前进程内 `traffic_up/down` 视为今日累计并落盘；同时处理跨日归档。
pub async fn record_tick(state: &AppState) {
    ...
    h.today_up = h.today_up.saturating_add(cur_up);
    h.today_down = h.today_down.saturating_add(cur_down);
